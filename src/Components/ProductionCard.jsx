...
import {Card, CardHeader, CardMedia, CardContent, Grid, Skeleton} from '@mui/material'

import {fetchProduction} from '../Actions/ProductionActions'

function ProductionCard({fetchProduction, productionId, production, relationships, ...props}) {
  useEffect(()=> {
  ...
  return (
    <Grid item xs={3}>
      <Card elevation={2}>
        <CardHeader title={production ? production.attributes.name : <Skeleton />} sx={{textAlign:'center'}}/>
